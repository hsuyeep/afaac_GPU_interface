...
class subbandHandler:
	...
	Nsec2Rd = -1; # -1 indicates read infinitely
	Nelem  = 288;       
	...
	_sel = numpy.nonzero(numpy.triu(numpy.ones([Nelem,Nelem])).flatten() == 1);
	...
	def readRec (self): # Can include number of records to read here TODO
		...
		self._vis_int = numpy.mean (self._vis, axis=2);
		self._acm [0][self._sel] = self._vis_int[:][0][1] + 1j*self._vis_int[:][0][2];
		self._acm [1][self._sel] = self._vis_int[:][1][1] + 1j*self._vis_int[:][1][2];
		self._acm [2][self._sel] = self._vis_int[:][2][1] + 1j*self._vis_int[:][2][2];
		self._acm [3][self._sel] = self._vis_int[:][3][1] + 1j*self._vis_int[:][3][2];
		

		return numpy.reshape (self._acm, [self.Npol, self.Nelem, self.Nelem]), self._tobs, self._fobs;
	
	# Function to integrate over specified channels
#	def accum (self, chrange):
#		self._acm_int = numpy.mean (self._acm, axis=2);
#
#	def gridVisibilities (self):
#
#
...
class imager:
	...
	def createImageDFT (self, acm, tobs):
...
if __name__ == '__main__':
	...
	while irec < subbandHandler.Nsec2Rd:
		...
		skymap = im.createImageDFT (acm, tobs);
