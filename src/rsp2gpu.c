...
int main (int argc, char *argv[])
{
  ...
  while (1)
  {
    ...
    for (i = 0; i < 4; i++)
    {
      ...
      if (max < rsppkt[i].hdr.timestamp)
        max = rsppkt[i].hdr.timestamp;
    }
      ...
      for (sb = 0; sb < NRUNISUBBANDS; sb++)
      {
        ...
        for (rsp = 0; rsp < 4; rsp++)
        {
          for (dip = 0; dip < 12; dip++)
          {
            ...
            { 
/*
 			  // Put all 48 dipoles, pol0 together
              // Re, pol 0
              sbdata[rsp * 24 + 2 * dip  ] = rsppkt[rsp].data[2 * dip  ][ts][0][0];
              // Im, pol 0
              sbdata[rsp * 24 + 2 * dip + 1] = rsppkt[rsp].data[2 * dip  ][ts][0][1];
              // Put all 48 dipoles, pol1 together
              // Re, pol 1
              sbdata[(rsp+4) * 24 + 2 * dip ] = rsppkt[rsp].data[2 * dip + 1][ts][0][0];
              // Im, pol 1
              sbdata[(rsp+4) * 24 + 2 * dip + 1] = rsppkt[rsp].data[2 * dip + 1][ts][0][1];
*/
              // Re, pol 0
              sbdata[rsp * 48 + 4 * dip  ] = rsppkt[rsp].data[2 * dip  ][ts][0][0];
              // Im, pol 0
              sbdata[rsp * 48 + 4 * dip + 1] =rsppkt[rsp].data[2 * dip  ][ts][0][1];
              // Re, pol 1
              sbdata[rsp * 48 + 4 * dip + 2] = rsppkt[rsp].data[2 * dip + 1][ts][0][0];
              // Im, pol 1
              sbdata[rsp * 48 + 4 * dip + 3] = rsppkt[rsp].data[2 * dip + 1][ts][0][1];
            }
              ...
              {
                // Re, pol 0
                sbdata[rsp * 48 + 4 * dip  ] = rsppkt[rsp].data[2 * dip  ][ts][1][0];
                // Im, pol 0
                sbdata[rsp * 48 + 4 * dip + 1] = rsppkt[rsp].data[2 * dip  ][ts][1][1];
                // Re, pol 1
                sbdata[rsp * 48 + 4 * dip + 2] = rsppkt[rsp].data[2 * dip + 1][ts][1][0];
                // Im, pol 1
                sbdata[rsp * 48 + 4 * dip + 3] = rsppkt[rsp].data[2 * dip + 1][ts][1][1];
              }

              else
              {
                sbdata[rsp * 48 + 4 * dip  ] = 0;
                sbdata[rsp * 48 + 4 * dip + 1] = 0;
                sbdata[rsp * 48 + 4 * dip + 2] = 0;
                sbdata[rsp * 48 + 4 * dip + 3] = 0;
              }
          }
        }
      }
      ...
      if ((ts % 2) == 1)
      {
        ...
        f2->f.field1 = (f1.f.field1);
        f2->f.field2 = (f1.f.field2);
        f2->f.field3 = (f1.f.field3);
        f2->f.field4 = (f1.f.field4);
