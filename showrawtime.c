...
/*
    for (int channel = 0; channel < nr_channels; channel ++) {
      complex float *ptr = &block[header_size / sizeof(complex float) + baseline * nr_channels * nr_polarizations + channel * nr_polarizations + polarization];
      printf("%u %u (%g,%g)\n", time, channel, creal(*ptr), cimag(*ptr));
    }
*/
